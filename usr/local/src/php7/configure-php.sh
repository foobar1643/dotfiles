### PRE INSTALL REQUIRED PACKAGES BEFORE CONFIGURING PHP ###
...
sudo apt-get install libfcgi-dev libfcgi0ldbl libbz2-dev libxml2-dev libmcrypt-dev libssl-dev libcurl4-openssl-dev zlib1g-dev libmhash-dev libc-client2007e libc-client2007e-dev libpng-dev libjpeg-dev libwebp-dev libfreetype6-dev libpq-dev libxslt1-dev libgmp-dev libkrb5-dev libldap2-dev openssl
...
# This will configure PHP to build with the following extensions:
  ...
  # openssl
  # curl
./configure --enable-fpm --enable-libxml --enable-bcmath --enable-calendar --enable-exif --enable-ftp --enable-sockets --enable-zip --enable-soap --enable-mbstring --enable-intl --with-pcre-regex --with-xsl --with-zlib --with-zlib-dir=/usr --with-bz2 --with-gd --enable-gd-native-ttf --with-webp-dir=/usr --with-jpeg-dir=/usr --with-png-dir=/usr --with-freetype-dir=/usr --with-gettext --with-gmp --with-mhash --with-imap --with-mcrypt --with-pgsql --with-pdo-pgsql --with-mysqli --with-pdo-mysql --with-pear --with-kerberos --with-imap-ssl --with-openssl --with-config-file-path=/etc/php/ --with-config-file-scan-dir=/etc/php/conf.d/ --with-curl --with-fpm-user=www-data --with-fpm-group=www-data --with-ldap --with-xmlrpc
